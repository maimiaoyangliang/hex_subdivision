...
void create_cylinder_with_hole() {
	...
	*/
	...
	for (i = 0; i < EDGE_NUM; ++i) {
		...
		hex_model.add_edge(i1, i2);
	}
	
	hex_model.set_acis_edge(geom_edge, EDGE_NUM);
	...
	*/
	...
	for (i = 0; i < FACE_NUM; ++i) {
		hex_model.add_face(edge_idx_in_face[i], 4);
	}
	
	hex_model.set_acis_face(acis_face, FACE_NUM);
	...
	for (i = 0; i < CELL_NUM; ++i) {
		hex_model.add_cell(face_idx_of_cell[i], 6);
	}
	
	ENTITY_LIST wl, wirelist;
