...
void hs_model::save_file( const char* filepath, bool bSplit, double coeff, 
						 double xlimit, double ylimit, double zlimit, const char* file_ext ) const {
	...
	if ( strcmp(file_ext, "obj") == 0 ) {
		if ( !bSplit ) {
			...
			for (size_t fidx = 0; fcitr != faces.end(); ++fcitr, ++fidx) {
				size_t vidx[4];
				sort_vert_of_face(vidx, fidx);
				fs << "f " << ++vidx[0] << " " << ++vidx[1] <<" " 
					<< ++vidx[2] << " " << ++vidx[3] << "\n";
			}
		} else {
			std::vector< hs_point > vcoord;
			std::vector< std::vector<size_t> > vidx_in_face;
			
			cell_vector_citer ccitr = cells.begin();
			for (; ccitr != cells.end(); ++ccitr) {
				hs_point cent;
				int_set_citer vcitr = ccitr->first_vert();
				for (; vcitr != ccitr->end_vert(); ++vcitr) {
					cent += vertices[*vcitr].coord();
				}
				cent /= vert_size();
				if ( cent.x() < xlimit && cent.y() < ylimit && cent.z() < zlimit) {
					int_set_citer fcitr = ccitr->first_face();
					for(; fcitr != ccitr->end_face(); ++fcitr) {
						const hs_face& face = faces[*fcitr];
						assert(face.vert_size() == 4);
						size_t vidx[4];
						std::vector< size_t > vidx_vec;
						sort_vert_of_face(vidx, *fcitr);
						for (size_t i = 0; i < 4; ++i) {
							hs_point diff = vertices[vidx[i]].coord() - cent;
							hs_point vert = cent + diff * coeff;
							vcoord.push_back(vert);
							vidx_vec.push_back(vcoord.size());
						}
						vidx_in_face.push_back(vidx_vec);
					}
				}
			} // end for
			
			std::vector< hs_point >::iterator vitr = vcoord.begin();
			for (; vitr != vcoord.end(); ++vitr) {
				fs << "v " << vitr->x()
					<< " " << vitr->y()
					<< " " << vitr->z()
					<< "\n";
			}
			std::vector< std::vector<size_t> >::iterator vif_itr= vidx_in_face.begin();
			for (; vif_itr != vidx_in_face.end(); ++vif_itr) {
				fs << "f ";
				std::vector< size_t >::iterator itr = vif_itr->begin();
				for (; itr != vif_itr->end(); ++itr) {
					fs << *itr << " ";
				}
				fs << "\n";
			}
...
void hs_model::sort_vert_of_face( size_t vidx[], size_t idx ) const {
