...
init_acis::init_acis(const std::string& rhs, bool m):cond(rhs), funMap(), modeller(m) {
	init_map();
	outcome res;
	if (modeller) {
...
std::string init_acis::create_string(const std::string& rhs) {
	return "api_" + rhs + "_" + cond;
}

void init_acis::check_result(const outcome& res, const std::string& funName) {
